        ...
        for y in range(0, self.img.height, 2):
            for x in range(self.img.width):
                ...
                if self.display_background:
                    yield Segment(
                        DOUBLE_PIXEL,
                        style=rich.style.Style(color=top_color, bgcolor=bottom_color),
                    )
                elif (
                    not top_color == BACKGROUND_COLOR
                    and not bottom_color == BACKGROUND_COLOR
                ):
                    yield Segment(
                        DOUBLE_PIXEL,
                        style=rich.style.Style(color=top_color, bgcolor=bottom_color),
                    ...
                    yield Segment(
                        DOUBLE_PIXEL_REVERSE, style=rich.style.Style(color=bottom_color)
                    )
                elif (
                    not top_color == BACKGROUND_COLOR
                    and bottom_color == BACKGROUND_COLOR
                ):
...
@click.option(
    ...
    "-l",
    default="en",
    help="Language to display the pokemon name / description",
)
@click.option(
    "--pokemon_id",
    "-p",
    ...
    multiple=True,
    type=int,
    help="Pokemon ids to display, can be multiple.",
)
@click.option(
    "--display-background",
    "-d",
    is_flag=True,
    help="Display the background of the image as black pixels.",
)
